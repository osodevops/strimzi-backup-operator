...
patch_image() { patch_cronjob_image "$(img_old)"; }
operator_uninstall
...
patch_image; r=$(wait_for 10 img_is "$(img_new)") || fail "image edit not reverted within 10s"
...
wait_for 120 img_is "$(img_new)" >/dev/null || fail "precondition (old)"
patch_image; sleep 60; [ "$(cronjob_image)" = "$(img_old)" ] || fail "negative control: 0.2.22 reverted the edit?"
